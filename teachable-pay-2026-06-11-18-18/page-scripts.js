...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const cookiePreferenceLink = document.getElementById('cookie-preference');

    // Check if the element exists
    if (cookiePreferenceLink) {
        // Attach a click event listener
        cookiePreferenceLink.addEventListener('click', function(event) {
            // Prevent default link behavior
            event.preventDefault();

            // Call the function to reshow the cookie notice
            hmReshowCookieNotice();
        });
    } else {
        console.warn('Element with id="cookie-preference" not found.');
    }
});

document.addEventListener('DOMContentLoaded', function() {
  var yearSpan = document.querySelector('.copy-year');
  if (yearSpan) { // <-- Added safe guard check
    var currentYear = new Date().getFullYear();
    yearSpan.textContent = currentYear;
  }
});

// Wait for Heap to be ready and check consent
function initScrollTracking() {
    // Check if user has consented (Hotmart sets this)
    if (typeof heap !== 'undefined' && !document.cookie.includes('cookie-policy-declined=true')) {
        ...
        function trackScrollDepth() {
            ...
            scrollDepthMarkers.forEach(function(marker) {
                if (currentDepth >= marker && !reachedMarkers.includes(marker)) {
                    reachedMarkers.push(marker);
                    
                    heap.track('Scroll Depth Reached', {
                        depth: marker + '%',
                        depthValue: marker,
                        pageTitle: document.title,
                        pagePath: window.location.pathname,
                        pageUrl: window.location.href,
                        isEuUser: localStorage.getItem('eu_user') === 'true'
                    });
                }
            });
        }
...
document.addEventListener("DOMContentLoaded", function() {
  // Find all email inputs EXCEPT those with data-no-required="true"
  document.querySelectorAll('input[type="email"]:not([data-no-required="true"])')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Check the user's browser setting
    const euLocales = ['de-AT', 'nl-BE', 'fr-BE', 'bg-BG', 'hr-HR', 'el-CY', 'cs-CZ', 'da-DK', 'et-EE', 'fi-FI', 'fr-FR', 'de-DE', 'el-GR', 'hu-HU', 'en-IE', 'it-IT', 'lv-LV', 'lt-LT', 'fr-LU', 'de-LU', 'mt-MT', 'nl-NL', 'pl-PL', 'pt-PT', 'ro-RO', 'sk-SK', 'sl-SI', 'es-ES', 'sv-SE'];
    const userLocale = navigator.language || navigator.userLanguage;
    const isEuUser = euLocales.includes(userLocale);

    // Set localStorage object for eu_user
    try {
        localStorage.setItem('eu_user', isEuUser);
    } catch (e) {
        console.error('Error setting item in localStorage:', e);
    }

    // Append HTML and modify input value if eu_user is true
    if (isEuUser) {
        // Inject CSS for proper visibility
        ...
        style.textContent = `
            ...
            .eu-disclaimer input[type="checkbox"] {
            ...
            /* Dark background detection and override */
            .eu-disclaimer {
        ...
        // Append HTML to forms with class 'form-content'
        const forms = document.querySelectorAll('.form-content');
        if (forms.length === 0) {
            console.warn('No forms with class "form-content" found.');
        } else {
            forms.forEach(form => {
                const euDisclaimerHTML = `
                    <label class="eu-disclaimer">
                        <input id="00N3t00000GH3mS" name="00N3t00000GH3mS" type="checkbox" opt-in-checkbox="true" 
                               title="I agree to receive product and marketing updates from Teachable" 
                               class="consent" value="true">
                        <span>I agree to receive product and marketing updates from Teachable</span>
                    </label>`;
                form.insertAdjacentHTML('beforeend', euDisclaimerHTML);
            });
        }

        // Modify the value of input with ID '00N3t00000GH4mE'
        const targetInput = document.getElementById('00N3t00000GH4mE');
        if (targetInput) {
            targetInput.value = 'true';
        } else {
            console.warn('Input with ID "00N3t00000GH4mE" not found.');
        }
    }

    // No action if eu_user is false
});

function updateUserUrlInInputs() {
    try {
        // Step 1: Capture the current URL
        var userUrlAttributionData = window.location.href;

        // Step 2: Find all input elements with the specific ID
        var inputs = document.querySelectorAll('input[id="00N3t00000GH4mJ"]');

        if (inputs.length > 0) {
            inputs.forEach(function(input) {
                input.value = userUrlAttributionData;
            });

            console.log(inputs.length + " inputs have been changed to the following URL: " + userUrlAttributionData);
        } else {
            console.log("No inputs were found, so no URLs were changed.");
        }
    } catch (error) {
        console.error("An error occurred while updating inputs: ", error);
    }
}

// Using addEventListener to avoid overwriting window.onload
document.addEventListener('DOMContentLoaded', updateUserUrlInInputs);
